...
export const MenuItemDrop = ({ link, variant = 'default' }) => {
  ...
  const linkBox = isInline
    ? 'rounded-md px-0 py-0 no-underline flex items-center gap-1.5 whitespace-nowrap'
  ...
  const linkClassName = isInline
    ? `tl-nav-link min-h-[44px] min-w-[44px] rounded-md px-2 py-1.5 no-underline flex items-center justify-center gap-1.5 whitespace-nowrap ${isActive ? 'is-active' : ''}`
    : undefined
